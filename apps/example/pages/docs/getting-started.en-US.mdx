...
`react-image-previewer` uses quite a lot of `CSS3` animations to ensure a smoother experience, and uses spring animations to stay close to the native scrolling effect when scrolling. `CSS3` animations have a default duration of `400ms`, using the `cubic-bezier(0.25, 0.8, 0.25, 1)` animation function.
...
`react-image-previewer` does not have a built-in rotation control button by default, but provides a rotation `API`. We can achieve the rotation effect by simply adding an `SVG` image:
